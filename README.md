...
در ابتدای پیاده‌سازی، الگوی Regex برای پارس کردن هر خط بیش از حد سخت‌گیرانه بود و برخی از خطوط فایل نمونه به دلیل ناقص بودن یا تفاوت‌های جزئی در فرمت، به درستی پردازش نمی‌شدند. در نتیجه، احتمال متوقف شدن برنامه هنگام برخورد با داده‌های نامعتبر وجود داشت.

برای حل این مشکل، فرآیند پارس به گونه‌ای تغییر داده شد که هر خط پس از استخراج اطلاعات اعتبارسنجی شود. اگر یکی از بخش‌های ضروری مانند IP، Timestamp، Status Code یا Endpoint معتبر نباشد، آن خط فقط به عنوان یک **Invalid Line** شمارش شده و پردازش ادامه پیدا می‌کند. این رویکرد باعث شد ابزار در برابر داده‌های واقعی و Dirty Data مقاوم باشد و بدون کرش کردن، کل فایل را پردازش کند.
